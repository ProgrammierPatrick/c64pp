...
void FloppyDrive::tick() {
    ...
    const std::array<uint8_t, 0x100> bamData = {
        ...
        0x0a, 0x0a, 0x0a, 0x0a, 0x0a, 0x32, 0x41, 0x0a, 0x0a, 0x0a, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    auto encodeGCR = [&gcr](auto pos, auto data) -> uint8_t {
        if ((pos % 5) == 0) {
            ...
            auto v0 = data[pos - 1];
            auto v1 = data[pos];
            if ((pos % 5) == 1)      return                       (gcr[v0 & 0x0F] << 6) | (gcr[v1 >> 4] << 1) | (gcr[v1 & 0x0F] >> 4);
            else if ((pos % 5) == 2) return                       (gcr[v0 & 0x0F] << 4) | (gcr[v1 >> 4] >> 1);
    ...
    if (bytePos >= 5 && bytePos < 15) {
        // header
        std::array<uint8_t,8> headerData { 0x08, 18, (uint8_t)sectorPos, 0x32, 0x41, static_cast<uint8_t>(18 ^ sectorPos ^ 0x32 ^ 0x41) ,0x0F, 0x0F };
    ...
    bool byteReady = !((tickCount % 26) == 0 && lastSOE);
    ...
    if (lastStepValue != step) std::cout << "1541 stepper motor: " << toHexStr(step) << std::endl;
    ...
    if (mpu.T == 0) {
        ...
        //    std::cout << "1541 [" << tickCount / 1'000'000.f << "] " << "SYSIRQ" << std::endl;
            ...
            std::cout << "1541 [" << tickCount / 1'000'000.f << "] " << "SEAK: read first byte" << std::endl;
        } if (mpu.PC == 0xF407) {
            std::cout << "1541 [" << tickCount / 1'000'000.f << "] " << "SEAK: not a header block" << std::endl;
        } if (mpu.PC == 0xF3C8) {
            std::cout << "1541 [" << tickCount / 1'000'000.f << "] " << "SEAK: read header byte" << std::endl;
        } if (mpu.PC == 0xF556) {
