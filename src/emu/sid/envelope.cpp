...
std::array<uint16_t,16> lfsr15CompareValLUT = {
...
uint8_t Envelope::tick() {
    ...
    bool resetLFSR5 = resetLFSR15 && (countingUp || fixedPointFound)
    ...
    if (!gate && counter == 0x00)
        countingEnabled = false;
    if (gate && counter > (sustain | (sustain << 4)))
        countingEnabled = true;
